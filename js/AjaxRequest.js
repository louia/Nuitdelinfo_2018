        ...
        if (RequestThis.transport.readyState === 4) {
            ...
            if (RequestThis.transport.status === 200) {
                ...
                switch (RequestThis.handleAs) {
                    case 'text' : result = RequestThis.transport.responseText;
                        break;
                    case 'json' : result = JSON.parse(RequestThis.transport.responseText);
                        break;
                    case 'xml' : result = RequestThis.transport.responseXML;
                        break;
                    default : result = RequestThis.transport.responseText; // Remettre peut être à null
                        break;
                }
